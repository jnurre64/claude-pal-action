...
run_adversarial_plan_review() {
    ...
    result=$(run_agent "$prompt" "$AGENT_ALLOWED_TOOLS_TRIAGE" "" "$AGENT_JSON_SCHEMA_ADVERSARIAL_PLAN" "ADVERSARIAL_PLAN")
...
run_test_gate() {
    ...
    while true; do
        ...
        result=$(run_agent "$prompt" "$impl_tools" "" "" "TEST_FIX")
...
run_post_impl_review() {
    ...
    result=$(run_agent "$prompt" "$AGENT_ALLOWED_TOOLS_TRIAGE" "" "$AGENT_JSON_SCHEMA_POST_IMPL_REVIEW" "POST_IMPL_REVIEW")
...
run_post_impl_retry_session() {
    ...
    result=$(run_agent "$prompt" "$impl_tools" "" "$AGENT_JSON_SCHEMA_POST_IMPL_RETRY" "POST_IMPL_RETRY")
